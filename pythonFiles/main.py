...
with open('./uniqueRatedISBN', 'rb') as file:
    uniqueISBNS = pickle.load(file)
...
def BookSimilarityInit(books_with_ratings):
    ...
    filtered_users = ratings_count[ratings_count['NumberOfBooks'] >= 5]
...
def get_recommendations_user_batch(isbn_batch):
    return [(isbn, FilteringCollaborationRecommendations(isbn, merged_dataset)) for isbn in isbn_batch]

if __name__ == '__main__':
    random_isbns = random.sample(uniqueISBNS, 80000)
    ...
    with ProcessPoolExecutor(max_workers=8) as executor:
        ...
        for future in as_completed(futures):
            ...
            # Progress tracking
            if (futures[future] + 1) % 10 == 0:  # Every 10 batches
                print(f"Reached batch index {(futures[future] + 1) * batch_size}")

    # Save the final recommendations list
    pickle.dump(halfBooksRecommendationList, open('./recommendationList_user', 'wb'))